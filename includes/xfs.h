...
{
    xfs_uint32_t	sb_magicnum;	/* magicnum идентифицирует фс. Для XFS это (0x58465342) */
    xfs_uint32_t	sb_blocksize;	/* размер базовой единицы выделяемого пространства в байтах */
    xfs_drfsbno_t	sb_dblocks;	/* number of data blocks */
    xfs_drfsbno_t	sb_rblocks;	/* number of realtime blocks */
    xfs_drtbno_t	sb_rextents;	/* number of realtime extents */
    uuid_t		sb_uuid;	/* file system unique id */
    xfs_dfsbno_t	sb_logstart;	/* starting block of log if internal */
    xfs_ino_t	sb_rootino;	/* номер корневого индекса файловой системы */
    ...
    xfs_uint16_t	sb_inodesize;	/* размер inode в байтах. Значение по умолчанию - 256 */
...
// структура с прямым порядком байт на диске
typedef struct xfs_bmbt_irec
{
    xfs_fileoff_t       br_startoff; // смещение логического блока
    xfs_fsblock_t       br_startblock; // блок файловой системы
 ...
 */
typedef struct { xfs_uint16_t i; } xfs_dir2_sf_off_t;
...
typedef struct __attribute__((packed)) xfs_dir2_sf_entry {
    xfs_uint8_t		namelen;	/* actual name length */
    xfs_uint16_t 	offset;		/* saved offset */
...
typedef struct xfs_dinode_core
{
    ...
    xfs_fsize_t	di_size;	/* размер файла в байтах */
    xfs_drfsbno_t	di_nblocks;	/* число блоков, необходимых для хранения данных inode # of direct & btree blocks used */
    xfs_extlen_t	di_extsize;	/* basic/minimum extent size for file */
    xfs_extnum_t	di_nextents;	/* количество экстентов в ответвлении данных */
    ...
    xfs_uint32_t    di_crc;
    xfs_uint64_t    di_changecount;
    xfs_uint64_t    di_lsn;
    xfs_uint64_t    di_flags2;
    xfs_uint32_t    di_cowextsize;
    xfs_uint8_t     di_pad2[12];
    xfs_timestamp_t di_crtime;
    xfs_uint64_t    di_ino; /* полный номер inode этого узла */
    uuid_t          di_uuid;
} xfs_dinode_core_t;

typedef struct xfs_dinode
{
    xfs_dinode_core_t	di_core; /*Он содержит информацию о самом файле, включая большую часть статистической информации о данных и ответвлениях атрибутов после ядра в inode*/
    union {
        xfs_bmdr_block_t di_bmbt;	/* btree root block */
        xfs_bmbt_rec_32_t di_bmx[1];	/* extent list */
        xfs_dir2_sf_t	di_dir2sf;	/* короткая форма для родительской директории */
        char		di_c[1];	/* local contents */
    } di_u; /*«ответвление данных» отслеживает блоки данных файла или блоки каталогов*/
} xfs_dinode_t;
